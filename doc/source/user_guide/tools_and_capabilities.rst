...
``run_python_script`` is also available for executing a complete Python file in the same persistent session. Depending on the size of the script, or the agent guidance, it may decide to use ``run_python_code`` instead of ``run_python_script`` to execute the script in smaller chunks.
